...
export function RentBuyResults({ inputs: i, result: r, onChange }: ResultsProps<RentBuyInputs, RentBuyResult>) {
  ...
  const stats = [
    { label: 'Year-1 monthly cost', value: `${fmtMoneyCompact(r.rent.firstYearMonthly)} vs ${fmtMoneyCompact(r.buy.monthlyOwnerCostYear1)}`, sub: 'rent vs own', help: 'Cash out the door each month in the first year. Owner cost includes mortgage, property tax, insurance, maintenance, HOA and PMI.' },
