...
import { brandCardCopy, OG_ROUTES, type OgRoute } from '../ogRoutes';

/** A calculator's own landing page: a result drawn from its defaults, with no preset named. */
const isCalculatorRoute = (r: OgRoute) => r.kind === 'result' && !r.presetId;

describe('OG_ROUTES', () => {
  it('covers every calculator, so a new one cannot ship without a link preview', () => {
    const covered = OG_ROUTES.filter(isCalculatorRoute).map((r) => r.calculatorId);
  ...
  it('points every result route at a calculator the registry can actually compute', () => {
    for (const route of OG_ROUTES.filter((r) => r.kind === 'result')) {
      // Comparison routes name a preset; both shapes must resolve to a computable calculator.
  ...
  it('uses paths that match the router, with no leading or trailing slash', () => {
    ...
    const calcPaths = OG_ROUTES.filter(isCalculatorRoute).map((r) => r.path);
